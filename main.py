...
def main():
    ...
    # ==========================================
    use_feature_engineering = config.get('features', {}).get('feature_engineering', {}).get('enabled', False)
    ...
    if use_feature_engineering:
        ...
        # Convert to DataFrame with current feature names
        X_train_df = pd.DataFrame(X_train, columns=feature_names)
        X_test_df = pd.DataFrame(X_test, columns=feature_names)
        
        # Create features
        X_train_df = engineer.create_features(X_train_df)
        X_test_df = engineer.create_features(X_test_df)
        ...
        print(f"  New shape after engineering: {X_train.shape}")
        print(f"  Total features: {len(feature_names)}")
    
    # ==========================================
    # STEP 5: HANDLE IMBALANCE (Optional)
    # ==========================================
    use_smote = config.get('training', {}).get('use_smote', False)
    
    if use_smote:
        print("\n" + "="*60)
        print("STEP 5: CLASS IMBALANCE CORRECTION")
        print("="*60)
